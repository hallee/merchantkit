...
class PurchaseTests : XCTestCase {
    ...
    func testNilIntroductoryOfferForUnknownSKProductSubscriptionPeriodUnit() {
        ...
        for subscriptionTestProduct in self.testProducts(areSubscriptions: true) {
            ...
            let introductoryOffer = MockSKProductDiscount(price: NSDecimalNumber(string: "0.00"), priceLocale: .current, subscriptionPeriod: mockSubscriptionPeriod, numberOfPeriods: 1, paymentMode: SKProductDiscount.PaymentMode(rawValue: 435345345)! /* unknown future case */)
